...
export function SearchableSelect({
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      >
        <span className={cn("flex-1 text-left truncate", !selectedItem && "text-muted-foreground")}>
      ...
      {open && (
        <div className="absolute z-50 left-0 top-full mt-1 min-w-full rounded-lg bg-popover text-popover-foreground shadow-md ring-1 ring-foreground/10 overflow-hidden">
          ...
          <div className="max-h-56 overflow-y-auto p-1">
            {filtered.length === 0 ? (
              <p className="py-2 text-center text-sm text-muted-foreground">Tidak ada hasil</p>
            ) : (
              filtered.map((item) => (
                <button
                  key={item.value}
