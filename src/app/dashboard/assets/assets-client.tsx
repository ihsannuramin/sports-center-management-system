...
const CONDITION_COLORS: Record<string, string> = {
  ...
  NEEDS_REPAIR: "bg-destructive/10 text-destructive",
};
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-4">
        <Card className="border-neutral shadow-sm"><CardContent className="p-4"><p className="text-xs text-muted-foreground mb-1">Total Aset</p><p className="text-2xl font-bold text-on-surface">{assets.length}</p></CardContent></Card>
        <Card className="border-neutral shadow-sm"><CardContent className="p-4"><p className="text-xs text-muted-foreground mb-1">Total Nilai</p><p className="text-xl font-bold text-on-surface">Rp {totalValue.toLocaleString("id-ID")}</p></CardContent></Card>
        <Card className="border-neutral shadow-sm"><CardContent className="p-4"><p className="text-xs text-muted-foreground mb-1">Perlu Perbaikan</p><p className="text-2xl font-bold text-destructive">{needsRepair}</p></CardContent></Card>
      </div>

      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-primary-10 rounded-xl"><Box className="w-5 h-5 text-primary" /></div>
          <div>
            <h2 className="font-semibold text-on-surface">Daftar Aset</h2>
            <p className="text-xs text-muted-foreground">{assets.length} aset terdaftar</p>
          </div>
        </div>
        <div className="flex gap-2 items-center">
          ...
          <Button size="sm" className="h-9 bg-primary hover:bg-primary-80 shadow-sm shadow-primary/20" onClick={openCreate}><Plus className="w-4 h-4 mr-1.5" /> Tambah Aset</Button>
        </div>
      </div>

      <div className="relative">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
        <Input placeholder="Cari aset..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="pl-9 h-9 border-border text-sm" />
      </div>

      {paginated.length === 0 ? (
        <div className="text-center py-16"><Box className="w-10 h-10 text-muted-foreground/30 mx-auto mb-3" /><p className="text-sm text-muted-foreground mb-3">Belum ada aset terdaftar</p><Button size="sm" className="bg-primary hover:bg-primary-80" onClick={openCreate}><Plus className="w-4 h-4 mr-1.5" /> Tambah Aset</Button></div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {paginated.map((asset) => (
            <Card key={asset.id} className="border-neutral shadow-sm hover:shadow-md transition-all">
              <CardContent className="p-4">
                ...
                {(asset.condition === "NEEDS_REPAIR" || asset.condition === "POOR") && (
                  <div className="mt-2 flex items-center gap-1.5 text-xs text-destructive bg-destructive/10 rounded-lg px-2.5 py-1.5">
                    <AlertCircle className="w-3.5 h-3.5" /> Perlu perhatian
                  </div>
                )}
                <div className="flex gap-3 mt-3 pt-3 border-t border-neutral">
                  <Button size="sm" variant="outline" className="flex-1 h-8 text-xs border-border cursor-pointer" onClick={() => openEdit(asset)}><Pencil className="w-3.5 h-3.5 mr-1.5" /> Edit</Button>
                  <Button size="sm" variant="outline" className="h-8 w-8 p-0 text-destructive border-destructive/20 hover:bg-destructive/10 cursor-pointer" title="Hapus" onClick={() => handleDelete(asset.id)}><Trash2 className="w-4 h-4" /></Button>
                </div>
              </CardContent>
            </Card>
          ))}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <form onSubmit={handleSubmit} className="space-y-3 pt-1">
            <div className="space-y-1.5"><Label className="text-xs font-medium text-foreground">Nama Aset *</Label><Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required /></div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5"><Label className="text-xs font-medium text-foreground">Tanggal Beli</Label><Input type="date" value={form.purchaseDate} onChange={(e) => setForm({ ...form, purchaseDate: e.target.value })} /></div>
              <div className="space-y-1.5"><Label className="text-xs font-medium text-foreground">Harga (Rp)</Label><Input type="number" min="0" value={form.cost} onChange={(e) => setForm({ ...form, cost: e.target.value })} /></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5"><Label className="text-xs font-medium text-foreground">Depresiasi (%/tahun)</Label><Input type="number" min="0" max="100" step="0.1" value={form.depreciation} onChange={(e) => setForm({ ...form, depreciation: e.target.value })} /></div>
              <div className="space-y-1.5"><Label className="text-xs font-medium text-foreground">Garansi s/d</Label><Input type="date" value={form.warrantyExpiry} onChange={(e) => setForm({ ...form, warrantyExpiry: e.target.value })} /></div>
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs font-medium text-foreground">Kondisi</Label>
              <select value={form.condition} onChange={(e) => setForm({ ...form, condition: e.target.value })} className="w-full h-9 border border-border rounded-lg px-3 text-sm bg-surface">
