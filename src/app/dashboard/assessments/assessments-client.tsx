      ...
      <CardHeader className="pb-4 border-b border-neutral">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-semibold text-on-surface">Penilaian Performa</h2>
              <p className="text-xs text-muted-foreground">{assessments.length} penilaian tersimpan</p>
            </div>
          </div>
        ...
        <Table className="table-row-hover">
          <TableHeader>
            <TableRow className="bg-muted/50 hover:bg-muted/50">
              <TableHead className="text-xs font-semibold text-tertiary pl-5">Siswa</TableHead>
              <TableHead className="text-xs font-semibold text-tertiary">Periode</TableHead>
              {skills.map(s => <TableHead key={s} className="text-xs font-semibold text-tertiary">{skillLabels[s]}</TableHead>)}
              <TableHead className="text-xs font-semibold text-tertiary">Rata-rata</TableHead>
                ...
                return (
                  <TableRow key={a.id}>
                    <TableCell className="pl-5">
                      <p className="font-medium text-sm text-on-surface">{a.student?.name}</p>
                    </TableCell>
                    <TableCell><span className="badge-blue">{a.period}</span></TableCell>
                    {skills.map((s) => (
                      <TableCell key={s}>
                        <div className="flex items-center gap-1.5">
                          <span className="text-sm font-semibold text-on-surface w-7 text-right">{a[s]}</span>
                    ...
                    <TableCell className="pr-3">
                      <Button variant="ghost" size="sm" className="h-7 text-xs text-tertiary hover:text-on-surface" onClick={() => openEdit(a)}>Edit</Button>
                    </TableCell>
                  </TableRow>
                );
              })
        ...
        <DialogContent className="max-w-lg">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 pt-1">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5"><Label className="text-xs font-medium text-foreground">Periode *</Label><Input value={form.period} onChange={(e) => setForm({ ...form, period: e.target.value })} placeholder="e.g. Q1-2026" required /></div>
            </div>
            <div className="bg-muted/50 rounded-xl p-4 space-y-3">
              <p className="text-xs font-semibold text-foreground uppercase tracking-wide">Nilai Skill (0–100)</p>
              <div className="grid grid-cols-2 gap-3">
                {skills.map((s) => (
                  <div key={s} className="space-y-1.5">
                    <div className="flex justify-between">
                      <Label className="text-xs font-medium text-foreground">{skillLabels[s]}</Label>
                      <span className="text-xs font-bold text-primary">{(form as any)[s]}</span>
                    </div>
