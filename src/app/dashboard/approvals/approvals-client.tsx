  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-4">
        <Card className="border-gray-50 shadow-sm"><CardContent className="p-4"><p className="text-xs text-gray-400 mb-1">Menunggu Persetujuan</p><p className="text-2xl font-bold text-yellow-600">{pendingCount}</p></CardContent></Card>
        <Card className="border-gray-50 shadow-sm"><CardContent className="p-4"><p className="text-xs text-gray-400 mb-1">Disetujui</p><p className="text-2xl font-bold text-green-600">{requests.filter(r => r.status === "APPROVED").length}</p></CardContent></Card>
        <Card className="border-gray-50 shadow-sm"><CardContent className="p-4"><p className="text-xs text-gray-400 mb-1">Ditolak</p><p className="text-2xl font-bold text-red-500">{requests.filter(r => r.status === "REJECTED").length}</p></CardContent></Card>
      </div>
      ...
      {paginated.length === 0 ? (
        ...
        <div className="space-y-3">
          {paginated.map((req) => (
            <Card key={req.id} className="border-gray-50 shadow-sm hover:shadow-md transition-all">
              <CardContent className="p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    {req.reason && <p className="text-xs text-gray-700 mb-1">{req.reason}</p>}
                    ...
                    {req.reviewNote && (
                      <div className="mt-2 bg-gray-50 rounded-lg px-3 py-2 text-xs text-gray-700">
