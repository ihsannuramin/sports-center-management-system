    ...
    <div className="space-y-5">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="border-neutral shadow-sm">
          <CardContent className="p-4">
            <p className="text-xs text-muted-foreground mb-1">Pending Pembayaran</p>
        ...
        <Card className="border-neutral shadow-sm">
          <CardContent className="p-4">
            <p className="text-xs text-muted-foreground mb-1">Total Pelatih</p>
            <p className="text-xl font-bold text-on-surface">{coaches.length}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-semibold text-on-surface">Daftar Payroll</h2>
            <p className="text-xs text-muted-foreground">{payrolls.length} record payroll</p>
          </div>
        </div>
        <div className="flex gap-2 items-center">
          ...
          <Button size="sm" className="h-9 bg-primary hover:bg-primary-80 shadow-sm shadow-primary/20" onClick={() => { setForm(emptyForm); setOpen(true); }}><Plus className="w-4 h-4 mr-1.5" /> Buat Payroll</Button>
        </div>
      </div>

      <div className="relative">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
        <Input placeholder="Cari pelatih atau periode..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="pl-9 h-9 border-border text-sm" />
      </div>

      {paginated.length === 0 ? (
        <div className="text-center py-16">
          <DollarSign className="w-10 h-10 text-muted-foreground/30 mx-auto mb-3" />
          <p className="text-sm text-muted-foreground">Belum ada data payroll</p>
        </div>
      ) : (
        <div className="rounded-xl border border-neutral overflow-hidden shadow-sm">
          <table className="w-full text-sm">
            <thead className="bg-muted/50 border-b border-neutral">
              <tr>
                <th className="text-left px-4 py-3 text-xs font-semibold text-tertiary uppercase tracking-wide">Pelatih</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-tertiary uppercase tracking-wide">Periode</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-tertiary uppercase tracking-wide">Tipe</th>
                <th className="text-right px-4 py-3 text-xs font-semibold text-tertiary uppercase tracking-wide">Total</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {paginated.map((p) => (
                <tr key={p.id} className="hover:bg-muted/50">
                  <td className="px-4 py-3 font-medium text-on-surface">{p.coach?.name}</td>
                  <td className="px-4 py-3 text-foreground">{p.period}</td>
                  <td className="px-4 py-3"><span className="text-xs bg-muted/50 text-foreground px-2 py-0.5 rounded-full">{p.payrollType}</span></td>
                  <td className="px-4 py-3 text-right font-semibold text-on-surface">Rp {Number(p.totalAmount).toLocaleString("id-ID")}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5">
                      ...
                      <button type="button" onClick={() => handleDelete(p.id)} title="Hapus" className="h-8 w-8 flex items-center justify-center rounded-lg hover:bg-destructive/10 text-muted-foreground/50 hover:text-destructive transition-colors cursor-pointer ml-1"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <DialogContent>
          ...
          <form onSubmit={handleSubmit} className="space-y-3 pt-1">
            <div className="space-y-1.5">
              ...
              <select value={form.coachId} onChange={(e) => setForm({ ...form, coachId: e.target.value })} required className="w-full h-9 border border-border rounded-lg px-3 text-sm bg-surface">
                <option value="">Pilih Pelatih</option>
                {coaches.map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5"><Label className="text-xs font-medium text-foreground">Periode *</Label><Input type="month" value={form.period} onChange={(e) => setForm({ ...form, period: e.target.value })} required /></div>
              <div className="space-y-1.5">
                <Label className="text-xs font-medium text-foreground">Tipe Payroll *</Label>
                <select value={form.payrollType} onChange={(e) => setForm({ ...form, payrollType: e.target.value })} className="w-full h-9 border border-border rounded-lg px-3 text-sm bg-surface">
                  {PAYROLL_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
                </select>
              </div>
            </div>
            {form.payrollType === "PER_SESSION" && (
              <div className="space-y-1.5"><Label className="text-xs font-medium text-foreground">Jumlah Sesi</Label><Input type="number" min="0" value={form.sessions} onChange={(e) => setForm({ ...form, sessions: e.target.value })} /></div>
            )}
            {form.payrollType === "PER_HOUR" && (
              <div className="space-y-1.5"><Label className="text-xs font-medium text-foreground">Jumlah Jam</Label><Input type="number" min="0" step="0.5" value={form.hours} onChange={(e) => setForm({ ...form, hours: e.target.value })} /></div>
            )}
            <div className="space-y-1.5"><Label className="text-xs font-medium text-foreground">Rate (Rp) *</Label><Input type="number" min="0" value={form.rateAmount} onChange={(e) => setForm({ ...form, rateAmount: e.target.value })} required /></div>
            <div className="space-y-1.5"><Label className="text-xs font-medium text-foreground">Catatan</Label><Input value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} /></div>
            <div className="flex justify-end gap-2 pt-2 border-t border-neutral">
              <Button type="button" variant="outline" onClick={() => setOpen(false)}>Batal</Button>
              <Button type="submit" className="bg-primary hover:bg-primary-80" disabled={loading}>{loading ? "Menyimpan..." : "Buat Payroll"}</Button>
            </div>
          </form>
        </DialogContent>
      </Dialog>
