    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        <div className="relative">
          ...
          <select value={catFilter} onChange={(e) => { setCatFilter(e.target.value); setPage(1); }} className="h-9 pl-9 pr-4 border border-gray-200 rounded-lg text-sm text-gray-700 bg-white">
            ...
            <tbody className="divide-y divide-gray-50">
              {paginated.map((expense) => (
                <tr key={expense.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3"><span className="text-xs bg-gray-50 text-gray-700 px-2 py-0.5 rounded-full">{expense.category}</span></td>
                    ...
                    <div className="flex items-center gap-1.5">
                      {expense.status === "PENDING" && (
                        <>
                          ...
                          <button type="button" onClick={() => handleReject(expense.id)} title="Tolak" className="h-8 w-8 flex items-center justify-center rounded-lg hover:bg-red-50 text-red-500 transition-colors cursor-pointer"><XCircle className="w-4 h-4" /></button>
                        </>
                      )}
                      <button type="button" onClick={() => handleDelete(expense.id)} title="Hapus" className="h-8 w-8 flex items-center justify-center rounded-lg hover:bg-red-50 text-gray-300 hover:text-red-500 transition-colors cursor-pointer ml-1"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
