...
const roleBadgeColor: Record<string, string> = {
  ...
  OPERATOR: "bg-orange-100 text-orange-500",
      ...
      <Tabs defaultValue="users">
                  ...
                  {paginated.length === 0 ? (
                    ...
                    paginated.map((user) => (
                      <TableRow key={user.id}>
                        ...
                        <TableCell>
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${roleBadgeColor[user.role?.name] ?? "bg-gray-100 text-gray-700"}`}>
                            {user.role?.name ?? "-"}
                          </span>
                        </TableCell>
                        <TableCell>
        ...
        <TabsContent value="roles" className="mt-5 space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {roles.map((role: any) => (
              <Card key={role.id} className="border-gray-50 shadow-sm hover:shadow-md transition-all">
      ...
      <Dialog open={permOpen} onOpenChange={setPermOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              Hak Akses —{" "}
              <span className={`text-sm px-2 py-0.5 rounded-full font-medium ${roleBadgeColor[permRole?.name] ?? "bg-gray-100 text-gray-700"}`}>
                {permRole?.name}
              </span>
            </DialogTitle>
          </DialogHeader>
