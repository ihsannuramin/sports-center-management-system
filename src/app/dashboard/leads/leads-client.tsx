    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-5 gap-3">
        {funnel.map((f) => (
          <button
            ...
            className={`p-3 rounded-xl border text-center transition-all ${stageFilter === f.stage ? "border-primary bg-primary-10" : "border-neutral bg-surface hover:border-primary-20"}`}
          >
            <p className="text-2xl font-bold text-on-surface">{f.count}</p>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-semibold text-on-surface">Daftar Lead</h2>
            <p className="text-xs text-muted-foreground">{leads.length} total lead</p>
          </div>
        </div>
        <div className="flex gap-2 items-center">
          ...
          <Button size="sm" className="h-9 bg-primary hover:bg-primary-80 shadow-sm shadow-primary/20" onClick={openCreate}><Plus className="w-4 h-4 mr-1.5" /> Tambah Lead</Button>
        </div>
      </div>
      ...
      {paginated.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-sm text-muted-foreground mb-3">Belum ada lead</p>
          <Button size="sm" className="bg-primary hover:bg-primary-80" onClick={openCreate}><Plus className="w-4 h-4 mr-1.5" /> Tambah Lead</Button>
        </div>
      ) : (
        <div className="rounded-xl border border-neutral overflow-hidden shadow-sm">
          <table className="w-full text-sm">
            <thead className="bg-muted/50 border-b border-neutral">
              <tr>
                <th className="text-left px-4 py-3 text-xs font-semibold text-tertiary uppercase tracking-wide">Nama</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-tertiary uppercase tracking-wide">Kontak</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-tertiary uppercase tracking-wide">Source</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-tertiary uppercase tracking-wide">Stage</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-tertiary uppercase tracking-wide">Cabang</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {paginated.map((lead) => (
                <tr key={lead.id} className="hover:bg-muted/50">
                  <td className="px-4 py-3 font-medium text-on-surface">{lead.fullName}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1 text-xs text-tertiary"><Phone className="w-3 h-3" /> {lead.phone}</div>
                    {lead.email && <div className="flex items-center gap-1 text-xs text-muted-foreground mt-0.5"><Mail className="w-3 h-3" /> {lead.email}</div>}
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-xs bg-muted/50 text-foreground px-2 py-0.5 rounded-full">{lead.source}</span>
                  </td>
                  <td className="px-4 py-3">
                    <select
                      value={lead.stage}
                  ...
                  <td className="px-4 py-3 text-xs text-tertiary">{lead.branch?.name ?? "-"}</td>
                  <td className="px-4 py-3">
                    <DropdownMenu>
                      <DropdownMenuTrigger render={<Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-foreground cursor-pointer" aria-label="Aksi" />}>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem onClick={() => handleDelete(lead.id)} className="cursor-pointer gap-2 text-destructive focus:text-destructive"><Trash2 className="w-3.5 h-3.5" /> Hapus</DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </td>
                </tr>
          ...
          <form onSubmit={handleSubmit} className="space-y-3 pt-1">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <select value={form.source} onChange={(e) => setForm({ ...form, source: e.target.value })} className="w-full h-9 border border-border rounded-lg px-3 text-sm bg-surface">
            ...
            <div className="space-y-1.5"><Label className="text-xs font-medium text-foreground">Catatan</Label><Textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} rows={2} /></div>
