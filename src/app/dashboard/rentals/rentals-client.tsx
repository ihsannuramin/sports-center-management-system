...
const statusColors: Record<string,string> = { PENDING:"bg-amber-100 text-amber-700", CONFIRMED:"bg-green-100 text-green-700", CANCELLED:"bg-muted text-foreground", COMPLETED:"bg-slate-100 text-slate-700" };
  ...
  return (
    <>
      <div className="flex items-center gap-2 flex-wrap">
        <SearchableSelect value={courtFilter} onValueChange={v=>handleCourtFilter(v??"ALL")} className="w-48 h-9 text-sm border-border">
        ...
        <Button size="sm" className="h-9 bg-primary hover:bg-primary-80 ml-auto" onClick={()=>{setForm(emptyF);setAvail(null);setCreateOpen(true);}}>
      ...
      <div className="flex gap-4 text-xs flex-wrap">
        {[{color:"bg-amber-500",label:"Menunggu"},{color:"bg-green-600",label:"Dikonfirmasi"},{color:"bg-slate-600",label:"Selesai"},{color:"bg-gray-400",label:"Dibatalkan"},{color:"bg-violet-600",label:"Jadwal Kelas"},{color:"bg-blue-600",label:"Latihan"},{color:"bg-red-600",label:"Perawatan"}].map(l=>(
          <div key={l.label} className="flex items-center gap-1.5"><span className={`w-2.5 h-2.5 rounded-sm ${l.color} inline-block`}/><span className="text-tertiary">{l.label}</span></div>
        ))}
      </div>

      <Tabs defaultValue="calendar">
        <TabsList className="bg-muted p-1 rounded-xl">
          <TabsTrigger value="calendar" className="rounded-lg text-sm data-[state=active]:bg-surface data-[state=active]:shadow-sm"><CalendarDays className="w-4 h-4 mr-1.5"/>Kalender</TabsTrigger>
          <TabsTrigger value="list" className="rounded-lg text-sm data-[state=active]:bg-surface data-[state=active]:shadow-sm"><List className="w-4 h-4 mr-1.5"/>Daftar Booking</TabsTrigger>
        </TabsList>

        <TabsContent value="calendar">
          <div className="bg-surface rounded-xl border border-neutral shadow-sm p-4 mt-4">
            <style>{`
          ...
          <p className="text-xs text-muted-foreground mt-2 text-center">Klik slot kosong untuk booking baru · Klik event untuk detail · Mode Agenda menampilkan semua event berurutan</p>
        </TabsContent>
          ...
          <Card className="border-neutral shadow-sm">
            <CardHeader className="pb-4 border-b border-neutral">
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <h2 className="font-semibold text-on-surface">Semua Booking <span className="text-muted-foreground font-normal text-sm">({filteredRentals.length})</span></h2>
                <div className="flex gap-2 items-center">
                  <SearchableSelect value={listFilter} onValueChange={v=>{setListFilter(v??"ALL");setPage(1);}} className="w-36 h-9 text-sm border-border">
                  ...
                  <Button variant="outline" size="sm" className="h-9 border-border text-foreground" onClick={()=>{const d=filteredRentals.map((r:any)=>({"No.Booking":r.bookingNumber,Pelanggan:r.customerName,"No.HP":r.customerPhone,Lapangan:r.court?.name,Tanggal:format(new Date(r.date),"dd/MM/yyyy"),"Jam Mulai":format(new Date(r.startTime),"HH:mm"),"Jam Selesai":format(new Date(r.endTime),"HH:mm"),"Durasi(jam)":r.duration,"Total(Rp)":Number(r.totalAmount),Status:statusLabels[r.status]}));exportToExcel(d,"Booking-Lapangan","Booking");toast.success(`${d.length} diekspor`);}}><Download className="w-4 h-4 mr-1.5"/>Excel</Button>
                </div>
              </div>
            </CardHeader>
            <CardContent className="p-0">
              <Table className="table-row-hover">
                <TableHeader><TableRow className="bg-muted/50 hover:bg-muted/50"><TableHead className="text-xs font-semibold text-tertiary pl-5">No. Booking</TableHead><TableHead className="text-xs font-semibold text-tertiary">Pelanggan</TableHead><TableHead className="text-xs font-semibold text-tertiary">Lapangan</TableHead><TableHead className="text-xs font-semibold text-tertiary">Tanggal & Waktu</TableHead><TableHead className="text-xs font-semibold text-tertiary">Total</TableHead><TableHead className="text-xs font-semibold text-tertiary">Status</TableHead><TableHead className="w-28"></TableHead></TableRow></TableHeader>
                <TableBody>
                  {paginated.length===0?(<TableRow><TableCell colSpan={7} className="text-center py-12 text-sm text-muted-foreground">Belum ada booking</TableCell></TableRow>):paginated.map((r:any)=>(
                    <TableRow key={r.id}>
                      <TableCell className="pl-5"><span className="font-mono text-xs text-tertiary bg-muted/50 px-2 py-1 rounded-md">{r.bookingNumber}</span></TableCell>
                      <TableCell><p className="font-medium text-sm text-on-surface">{r.customerName}</p><p className="text-xs text-muted-foreground">{r.customerPhone}</p></TableCell>
                      <TableCell className="text-sm text-foreground">{r.court?.name}</TableCell>
                      <TableCell><p className="text-sm text-foreground">{format(new Date(r.date),"d MMM yyyy")}</p><p className="text-xs text-muted-foreground">{format(new Date(r.startTime),"HH:mm")}–{format(new Date(r.endTime),"HH:mm")} ({r.duration}j)</p></TableCell>
                      <TableCell className="text-sm font-semibold text-on-surface">Rp {Number(r.totalAmount).toLocaleString("id-ID")}</TableCell>
                      <TableCell><span className={statusBadge[r.status]}>{statusLabels[r.status]}</span></TableCell>
                      <TableCell className="pr-3">
                        {r.status==="PENDING"&&<div className="flex gap-1"><Button size="sm" onClick={()=>handleStatus(r.id,"CONFIRMED")} className="bg-green-500 hover:bg-green-600 h-7 text-xs">Konfirmasi</Button><Button size="sm" variant="outline" onClick={()=>handleStatus(r.id,"CANCELLED")} className="text-destructive h-7 text-xs">Batal</Button></div>}
        ...
        <DialogContent className="max-w-lg">
          ...
          <form onSubmit={handleCreate} className="space-y-3 pt-1">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5"><Label className="text-xs font-medium text-foreground">Lapangan *</Label><SearchableSelect value={form.courtId} onValueChange={v=>{if(!v)return;const c=courts.find((x:any)=>x.id===v);setForm({...form,courtId:v,branchId:c?.branchId||""});setAvail(null);}} placeholder="Pilih lapangan">{courts.filter((c:any)=>c.isActive).map((c:any)=><SearchableSelectItem key={c.id} value={c.id}>{c.name}</SearchableSelectItem>)}</SearchableSelect></div>
              <div className="space-y-1.5"><Label className="text-xs font-medium text-foreground">Tanggal *</Label><Input type="date" value={form.date} onChange={e=>{setForm({...form,date:e.target.value});setAvail(null);}} required/></div>
              <div className="space-y-1.5"><Label className="text-xs font-medium text-foreground">Jam Mulai *</Label><Input type="time" value={form.startTime} onChange={e=>{setForm({...form,startTime:e.target.value});setAvail(null);}} required/></div>
              <div className="space-y-1.5"><Label className="text-xs font-medium text-foreground">Jam Selesai *</Label><Input type="time" value={form.endTime} onChange={e=>{setForm({...form,endTime:e.target.value});setAvail(null);}} required/></div>
            </div>
            ...
            {avail&&<div className={`flex items-center gap-2 p-3 rounded-xl text-sm ${avail.available?"bg-green-50 text-green-700 border border-green-200":"bg-destructive/10 text-destructive border border-destructive/20"}`}>{avail.available?<><CheckCircle className="w-4 h-4"/>Lapangan tersedia</>:<><AlertCircle className="w-4 h-4"/>{avail.conflictClass?`Jadwal kelas "${avail.conflictClass.name}" aktif pada waktu ini`:avail.conflictSchedule?.type==="MAINTENANCE"?"Lapangan dalam perawatan":avail.conflictSchedule?.type==="ACADEMY"?"Jadwal latihan akademi aktif":"Lapangan tidak tersedia"}</>}</div>}
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2 space-y-1.5"><Label className="text-xs font-medium text-foreground">Nama Pelanggan *</Label><Input value={form.customerName} onChange={e=>setForm({...form,customerName:e.target.value})} required/></div>
              <div className="space-y-1.5"><Label className="text-xs font-medium text-foreground">No. HP *</Label><Input value={form.customerPhone} onChange={e=>setForm({...form,customerPhone:e.target.value})} required/></div>
              <div className="space-y-1.5"><Label className="text-xs font-medium text-foreground">Harga/Jam (Rp) *</Label><Input type="number" value={form.pricePerHour||""} onChange={e=>setForm({...form,pricePerHour:Number(e.target.value)})} required/></div>
            </div>
            {dur>0&&form.pricePerHour>0&&<div className="bg-primary-10 border border-primary-20 p-3 rounded-xl text-sm space-y-1"><div className="flex justify-between"><span className="text-tertiary">Lapangan</span><span className="font-medium">{selCourt?.name||"—"}</span></div><div className="flex justify-between"><span className="text-tertiary">Durasi</span><span>{dur} jam</span></div><div className="flex justify-between border-t pt-1 font-semibold text-primary"><span>Total</span><span>Rp {total.toLocaleString("id-ID")}</span></div></div>}
            <div className="flex justify-end gap-2 pt-2 border-t border-neutral"><Button type="button" variant="outline" onClick={()=>setCreateOpen(false)}>Batal</Button><Button type="submit" className="bg-primary hover:bg-primary-80" disabled={loading||(avail!==null&&!avail.available)}>{loading?"Menyimpan...":"Buat Booking"}</Button></div>
          </form>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="max-w-sm">
          ...
          {detailEvent && !detailEvent._nonRental && (
            <div className="space-y-3 text-sm">
              <div className="flex items-center justify-between">
                <span className="font-mono text-xs text-muted-foreground">{detailEvent.bookingNumber}</span>
                <span className={statusBadge[detailEvent.status]}>{statusLabels[detailEvent.status]}</span>
              </div>
              <div className="space-y-2 border border-neutral rounded-xl p-3 bg-muted/50">
                {[["Lapangan",detailEvent.court],["Tanggal",detailEvent.date?format(new Date(detailEvent.date),"d MMM yyyy",{locale:idLocale}):"—"],["Waktu",detailEvent._start?`${format(new Date(detailEvent._start),"HH:mm")} – ${format(new Date(detailEvent._end),"HH:mm")}`:"—"],["Pelanggan",detailEvent.customer],["No. HP",detailEvent.phone||"—"],["Total",`Rp ${detailEvent.totalAmount?.toLocaleString("id-ID")}`]].map(([k,v])=>(
                  <div key={k} className="flex justify-between gap-4"><span className="text-tertiary shrink-0">{k}</span><span className="font-medium text-on-surface text-right">{v}</span></div>
                ))}
              </div>
              {detailEvent.status==="PENDING"&&<div className="flex gap-2"><Button className="flex-1 bg-green-500 hover:bg-green-600" onClick={()=>handleStatus(detailEvent.rentalId,"CONFIRMED")}>Konfirmasi</Button><Button variant="outline" className="flex-1 text-destructive border-destructive/20" onClick={()=>handleStatus(detailEvent.rentalId,"CANCELLED")}>Batalkan</Button></div>}
          ...
          {detailEvent?._nonRental && (
            <div className="space-y-3 text-sm">
              ...
              <p className="text-xs text-muted-foreground text-center">Event ini tidak dapat diubah dari halaman ini</p>
            </div>
          )}
        </DialogContent>
      </Dialog>
