...
const statusColors: Record<string,string> = { PENDING:"bg-amber-100 text-amber-700", CONFIRMED:"bg-green-100 text-green-700", CANCELLED:"bg-gray-100 text-gray-700", COMPLETED:"bg-slate-100 text-slate-700" };
    ...
    <>
      ...
      <Tabs defaultValue="calendar">
        ...
        <TabsContent value="calendar">
          <div className="bg-white rounded-xl border border-gray-50 shadow-sm p-4 mt-4">
            <style>{`
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-w-lg">
          ...
          <form onSubmit={handleCreate} className="space-y-3 pt-1">
            ...
            {dur>0&&form.pricePerHour>0&&<div className="bg-orange-50 border border-orange-200 p-3 rounded-xl text-sm space-y-1"><div className="flex justify-between"><span className="text-gray-500">Lapangan</span><span className="font-medium">{selCourt?.name||"—"}</span></div><div className="flex justify-between"><span className="text-gray-500">Durasi</span><span>{dur} jam</span></div><div className="flex justify-between border-t pt-1 font-semibold text-orange-500"><span>Total</span><span>Rp {total.toLocaleString("id-ID")}</span></div></div>}
            <div className="flex justify-end gap-2 pt-2 border-t border-gray-50"><Button type="button" variant="outline" onClick={()=>setCreateOpen(false)}>Batal</Button><Button type="submit" className="bg-orange-500 hover:bg-orange-600" disabled={loading||(avail!==null&&!avail.available)}>{loading?"Menyimpan...":"Buat Booking"}</Button></div>
          </form>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="max-w-sm">
          ...
          {detailEvent && !detailEvent._nonRental && (
            <div className="space-y-3 text-sm">
              ...
              <div className="space-y-2 border border-gray-50 rounded-xl p-3 bg-gray-50">
