    ...
    <div className="space-y-5">
      ...
      {paginated.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {paginated.map((p) => (
            <Card key={p.id} className="border-gray-50 shadow-sm hover:shadow-md transition-all cursor-pointer" onClick={() => setDetailParent(p)}>
              <CardContent className="p-4">
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 bg-gradient-to-br from-primary-60 to-orange-600 rounded-xl flex items-center justify-center text-white font-bold flex-shrink-0">
                    {p.fullName[0]}
                  </div>
                  ...
                  {(detailParent.parentCommunications ?? []).map((c: any) => (
                    <div key={c.id} className="bg-gray-50 rounded-lg px-3 py-2">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-xs font-medium text-orange-500">{c.channel}</span>
