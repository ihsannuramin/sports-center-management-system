      ...
      <Tabs defaultValue="plans">
        ...
        <TabsContent value="plans" className="mt-5 space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {plans.map((plan) => (
              <Card key={plan.id} className={`border-gray-50 shadow-sm ${!plan.isActive ? "opacity-60" : ""}`}>
              ...
              <tbody className="divide-y divide-gray-50">
                {filteredMemberships.map((m) => (
                  <tr key={m.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-700 text-xs">{m.plan?.name} <br /><span className="text-gray-400">{m.plan?.type}</span></td>
        ...
        <TabsContent value="rental" className="mt-5 space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {packages.map((pkg) => (
              <Card key={pkg.id} className={`border-gray-50 shadow-sm ${!pkg.isActive ? "opacity-60" : ""}`}>
                <CardContent className="p-4">
