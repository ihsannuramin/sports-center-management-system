  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-4">
        <Card className="border-gray-50 shadow-sm"><CardContent className="p-4"><p className="text-xs text-gray-400 mb-1">Total Promo</p><p className="text-2xl font-bold text-gray-900">{promos.length}</p></CardContent></Card>
        <Card className="border-gray-50 shadow-sm"><CardContent className="p-4"><p className="text-xs text-gray-400 mb-1">Promo Aktif</p><p className="text-2xl font-bold text-green-600">{activeCount}</p></CardContent></Card>
        <Card className="border-gray-50 shadow-sm"><CardContent className="p-4"><p className="text-xs text-gray-400 mb-1">Total Digunakan</p><p className="text-2xl font-bold text-orange-500">{promos.reduce((s, p) => s + p.usedCount, 0)}</p></CardContent></Card>
      </div>

      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-orange-50 rounded-md"><Tag className="w-5 h-5 text-orange-500" /></div>
      ...
      {paginated.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {paginated.map((promo) => (
            <Card key={promo.id} className={`border-gray-50 shadow-sm hover:shadow-md transition-all ${!promo.isActive ? "opacity-70" : ""}`}>
              <CardContent className="p-4">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <p className="font-semibold text-gray-900">{promo.name}</p>
                    <button
                      className="flex items-center gap-1 mt-1 text-sm font-mono font-bold text-orange-500 hover:text-orange-500"
          ...
          <form onSubmit={handleSubmit} className="space-y-3 pt-1">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap gap-2">
                {CONDITIONS.map((c) => (
                  <button key={c.key} type="button" onClick={() => toggleCondition(c.key)} className={`text-xs px-3 py-1.5 rounded-lg border transition-all ${form.conditions.includes(c.key) ? "bg-orange-50 border-orange-200 text-orange-500" : "border-gray-200 text-gray-700 hover:border-gray-300"}`}>{c.label}</button>
                ))}
              </div>
            </div>
