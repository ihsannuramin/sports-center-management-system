...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  PAYMENT_RECEIPT: "bg-orange-50 text-orange-500",
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-4 gap-4">
        {CATEGORIES.map((cat) => (
          <Card key={cat} className="border-gray-50 shadow-sm cursor-pointer hover:shadow-md transition-all" onClick={() => setCatFilter(catFilter === cat ? "" : cat)}>
      ...
      {paginated.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {paginated.map((doc) => (
            <Card key={doc.id} className="border-gray-50 shadow-sm hover:shadow-md transition-all group">
              <CardContent className="p-4">
