...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  PAYMENT_RECEIPT: "bg-primary-10 text-primary",
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-4 gap-4">
        {CATEGORIES.map((cat) => (
          <Card key={cat} className="border-neutral shadow-sm cursor-pointer hover:shadow-md transition-all" onClick={() => setCatFilter(catFilter === cat ? "" : cat)}>
            <CardContent className="p-4">
              <p className="text-xs text-muted-foreground mb-1">{CATEGORY_LABELS[cat]}</p>
              <p className="text-2xl font-bold text-on-surface">{documents.filter(d => d.category === cat).length}</p>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div><h2 className="font-semibold text-on-surface">Manajemen Dokumen</h2><p className="text-xs text-muted-foreground">{documents.length} dokumen tersimpan</p></div>
        </div>
        <Button size="sm" className="h-9 bg-primary hover:bg-primary-80 shadow-sm shadow-primary/20" onClick={() => { setForm(emptyForm); setOpen(true); }}><Plus className="w-4 h-4 mr-1.5" /> Upload Dokumen</Button>
      </div>

      <div className="flex gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
          <Input placeholder="Cari dokumen..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="pl-9 h-9 border-border text-sm" />
        </div>
      ...
      {paginated.length === 0 ? (
        <div className="text-center py-16"><FileText className="w-10 h-10 text-muted-foreground/30 mx-auto mb-3" /><p className="text-sm text-muted-foreground mb-3">Belum ada dokumen</p><Button size="sm" className="bg-primary hover:bg-primary-80" onClick={() => { setForm(emptyForm); setOpen(true); }}><Plus className="w-4 h-4 mr-1.5" /> Upload Dokumen</Button></div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {paginated.map((doc) => (
            <Card key={doc.id} className="border-neutral shadow-sm hover:shadow-md transition-all group">
              <CardContent className="p-4">
                <div className="flex items-start gap-3 mb-3">
                  <div className="w-10 h-12 bg-muted rounded-lg flex items-center justify-center flex-shrink-0">
                ...
                <div className="flex gap-3">
                  <a href={doc.fileUrl} target="_blank" rel="noopener noreferrer" className="flex-1">
                    <Button size="sm" variant="outline" className="w-full h-8 text-xs border-border hover:border-foreground/20 cursor-pointer"><ExternalLink className="w-3.5 h-3.5 mr-1.5" /> Buka</Button>
                  </a>
                  <Button size="sm" variant="outline" className="h-8 w-8 p-0 text-destructive border-destructive/20 hover:bg-destructive/10 cursor-pointer" title="Hapus" onClick={() => handleDelete(doc.id)}><Trash2 className="w-4 h-4" /></Button>
                </div>
              </CardContent>
            </Card>
          ))}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <form onSubmit={handleSubmit} className="space-y-3 pt-1">
            <div className="space-y-1.5"><Label className="text-xs font-medium text-foreground">Nama Dokumen *</Label><Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required /></div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label className="text-xs font-medium text-foreground">Tipe File</Label>
            ...
            <div className="space-y-1.5"><Label className="text-xs font-medium text-foreground">URL File *</Label><Input value={form.fileUrl} onChange={(e) => setForm({ ...form, fileUrl: e.target.value })} placeholder="https://..." required /></div>
            <div className="space-y-1.5">
