  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-4 gap-4">
        <Card className="border-gray-50 shadow-sm"><CardContent className="p-4"><p className="text-xs text-gray-400 mb-1">Total</p><p className="text-2xl font-bold text-gray-900">{notifications.length}</p></CardContent></Card>
        <Card className="border-gray-50 shadow-sm"><CardContent className="p-4"><p className="text-xs text-gray-400 mb-1">Belum Dibaca</p><p className="text-2xl font-bold text-orange-500">{unreadCount}</p></CardContent></Card>
        <Card className="border-gray-50 shadow-sm"><CardContent className="p-4"><p className="text-xs text-gray-400 mb-1">Terkirim</p><p className="text-2xl font-bold text-blue-600">{notifications.filter(n => n.status === "SENT").length}</p></CardContent></Card>
        <Card className="border-gray-50 shadow-sm"><CardContent className="p-4"><p className="text-xs text-gray-400 mb-1">Gagal</p><p className="text-2xl font-bold text-red-500">{notifications.filter(n => n.status === "FAILED").length}</p></CardContent></Card>
      </div>

      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-orange-50 rounded-xl"><Bell className="w-5 h-5 text-orange-500" /></div>
          <div><h2 className="font-semibold text-gray-900">Notification Center</h2><p className="text-xs text-gray-400">{notifications.length} total notifikasi</p></div>
        </div>
        <div className="flex gap-2 items-center">
          {unreadCount > 0 && (
            <Button variant="outline" size="sm" className="h-9 border-gray-200 text-gray-700" onClick={handleMarkAllRead}><CheckCheck className="w-4 h-4 mr-1.5" /> Tandai Semua</Button>
          )}
          <Button size="sm" className="h-9 bg-orange-500 hover:bg-orange-600 shadow-sm shadow-orange-200" onClick={() => { setForm(emptyForm); setOpen(true); }}><Plus className="w-4 h-4 mr-1.5" /> Buat Notifikasi</Button>
        </div>
      </div>
      ...
      <div className="flex gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <Input placeholder="Cari notifikasi..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="pl-9 h-9 border-gray-200 text-sm" />
        </div>
        <select value={channelFilter} onChange={(e) => { setChannelFilter(e.target.value); setPage(1); }} className="h-9 px-3 border border-gray-200 rounded-lg text-sm text-gray-700 bg-white">
      ...
      {paginated.length === 0 ? (
        <div className="text-center py-16"><Bell className="w-10 h-10 text-gray-200 mx-auto mb-3" /><p className="text-sm text-gray-400">Belum ada notifikasi</p></div>
      ) : (
        <div className="space-y-2">
          {paginated.map((notif) => {
            const ChannelIcon = CHANNEL_ICONS[notif.channel] ?? Bell;
            return (
              <Card key={notif.id} className={`border-gray-50 shadow-sm hover:shadow-md transition-all ${notif.status === "READ" ? "opacity-70" : ""}`}>
                <CardContent className="p-4">
                  <div className="flex items-start gap-3">
                    <div className={`p-2 rounded-xl flex-shrink-0 ${CHANNEL_COLORS[notif.channel] ?? "bg-gray-50 text-gray-500"}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-2 text-xs text-gray-400">
                        <span className="bg-gray-50 px-2 py-0.5 rounded text-gray-500">{notif.type}</span>
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      ...
                      <button type="button" onClick={() => handleDelete(notif.id)} title="Hapus" className="h-8 w-8 flex items-center justify-center rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 transition-colors cursor-pointer"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </div>
                </CardContent>
              </Card>
          ...
          <form onSubmit={handleSubmit} className="space-y-3 pt-1">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <select value={form.channel} onChange={(e) => setForm({ ...form, channel: e.target.value })} className="w-full h-9 border border-gray-200 rounded-lg px-3 text-sm bg-white">
                  <option value="IN_APP">In App</option>
                  <option value="EMAIL">Email</option>
                  <option value="WHATSAPP">WhatsApp</option>
                </select>
