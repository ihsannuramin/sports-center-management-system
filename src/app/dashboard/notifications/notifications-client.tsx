  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-4 gap-4">
        <Card className="border-gray-100 shadow-sm"><CardContent className="p-4"><p className="text-xs text-gray-400 mb-1">Total</p><p className="text-2xl font-bold text-gray-900">{notifications.length}</p></CardContent></Card>
        <Card className="border-gray-100 shadow-sm"><CardContent className="p-4"><p className="text-xs text-gray-400 mb-1">Belum Dibaca</p><p className="text-2xl font-bold text-orange-500">{unreadCount}</p></CardContent></Card>
        <Card className="border-gray-100 shadow-sm"><CardContent className="p-4"><p className="text-xs text-gray-400 mb-1">Terkirim</p><p className="text-2xl font-bold text-blue-600">{notifications.filter(n => n.status === "SENT").length}</p></CardContent></Card>
        <Card className="border-gray-100 shadow-sm"><CardContent className="p-4"><p className="text-xs text-gray-400 mb-1">Gagal</p><p className="text-2xl font-bold text-red-500">{notifications.filter(n => n.status === "FAILED").length}</p></CardContent></Card>
      </div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2 items-center">
          {unreadCount > 0 && (
            <Button variant="outline" size="sm" className="h-9 border-gray-200 text-gray-600" onClick={handleMarkAllRead}><CheckCheck className="w-4 h-4 mr-1.5" /> Tandai Semua</Button>
          )}
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        <select value={channelFilter} onChange={(e) => { setChannelFilter(e.target.value); setPage(1); }} className="h-9 px-3 border border-gray-200 rounded-lg text-sm text-gray-600 bg-white">
      ...
      {paginated.length === 0 ? (
        ...
        <div className="space-y-2">
          {paginated.map((notif) => {
            ...
            return (
              <Card key={notif.id} className={`border-gray-100 shadow-sm hover:shadow-md transition-all ${notif.status === "READ" ? "opacity-70" : ""}`}>
