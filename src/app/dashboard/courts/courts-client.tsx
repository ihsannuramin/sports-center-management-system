              ...
              <TableBody>
                {pc.length===0?(<TableRow><TableCell colSpan={6} className="text-center py-12 text-sm text-gray-400">Belum ada lapangan</TableCell></TableRow>):pc.map(c=>(
                  <TableRow key={c.id}>
                    ...
                    <TableCell className="text-sm text-gray-700">{c.branch?.name}</TableCell>
              ...
              <TableBody>
                {ps.length===0?(<TableRow><TableCell colSpan={5} className="text-center py-12 text-sm text-gray-400">Belum ada jadwal blokir</TableCell></TableRow>):ps.map(s=>(
                  <TableRow key={s.id}>
                    ...
                    <TableCell className="text-sm text-gray-700">{format(new Date(s.startTime),"d MMM yyyy HH:mm")}</TableCell>
                    <TableCell className="text-sm text-gray-700">{format(new Date(s.endTime),"d MMM yyyy HH:mm")}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
      ...
      <Dialog open={cOpen} onOpenChange={setCOpen}>
        <DialogContent><DialogHeader><DialogTitle>Tambah Lapangan</DialogTitle></DialogHeader>
          <form onSubmit={submitCourt} className="space-y-3 pt-1">
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-gray-50"><Button type="button" variant="outline" onClick={()=>setCOpen(false)}>Batal</Button><Button type="submit" className="bg-orange-500 hover:bg-orange-600" disabled={loading}>{loading?"Menyimpan...":"Simpan"}</Button></div>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={sOpen} onOpenChange={setSOpen}>
        <DialogContent><DialogHeader><DialogTitle>Tambah Jadwal Blokir</DialogTitle></DialogHeader>
          <form onSubmit={submitSched} className="space-y-3 pt-1">
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-gray-50"><Button type="button" variant="outline" onClick={()=>setSOpen(false)}>Batal</Button><Button type="submit" className="bg-orange-500 hover:bg-orange-600" disabled={loading}>{loading?"Menyimpan...":"Simpan"}</Button></div>
          </form>
        </DialogContent>
      </Dialog>
    </Tabs>
