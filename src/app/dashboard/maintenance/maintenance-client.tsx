  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-4">
        <Card className="border-gray-50 shadow-sm"><CardContent className="p-4"><p className="text-xs text-gray-400 mb-1">Tiket Terbuka</p><p className="text-2xl font-bold text-yellow-600">{openCount}</p></CardContent></Card>
        <Card className="border-gray-50 shadow-sm"><CardContent className="p-4"><p className="text-xs text-gray-400 mb-1">Dalam Proses</p><p className="text-2xl font-bold text-blue-600">{inProgressCount}</p></CardContent></Card>
        <Card className="border-gray-50 shadow-sm"><CardContent className="p-4"><p className="text-xs text-gray-400 mb-1">Urgent</p><p className="text-2xl font-bold text-red-500">{urgentCount}</p></CardContent></Card>
      </div>
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="h-9 px-3 border border-gray-200 rounded-lg text-sm text-gray-700 bg-white">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map((ticket) => {
            ...
            return (
              <Card key={ticket.id} className="border-gray-50 shadow-sm hover:shadow-md transition-all">
                <CardContent className="p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      ...
                      <button type="button" onClick={() => handleDelete(ticket.id)} title="Hapus" className="h-8 w-8 flex items-center justify-center rounded-lg bg-red-50 text-red-500 hover:bg-red-100 transition-colors cursor-pointer ml-1"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </div>
                </CardContent>
              </Card>
