  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-4">
        <Card className="border-neutral shadow-sm"><CardContent className="p-4"><p className="text-xs text-muted-foreground mb-1">Tiket Terbuka</p><p className="text-2xl font-bold text-yellow-600">{openCount}</p></CardContent></Card>
        <Card className="border-neutral shadow-sm"><CardContent className="p-4"><p className="text-xs text-muted-foreground mb-1">Dalam Proses</p><p className="text-2xl font-bold text-blue-600">{inProgressCount}</p></CardContent></Card>
        <Card className="border-neutral shadow-sm"><CardContent className="p-4"><p className="text-xs text-muted-foreground mb-1">Urgent</p><p className="text-2xl font-bold text-destructive">{urgentCount}</p></CardContent></Card>
      </div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-semibold text-on-surface">Tiket Maintenance</h2>
            <p className="text-xs text-muted-foreground">{tickets.length} total tiket</p>
          </div>
        </div>
        <Button size="sm" className="h-9 bg-primary hover:bg-primary-80 shadow-sm shadow-primary/20" onClick={() => { setForm(emptyForm); setOpen(true); }}><Plus className="w-4 h-4 mr-1.5" /> Buat Tiket</Button>
      </div>

      <div className="flex gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
          <Input placeholder="Cari tiket..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 h-9 border-border text-sm" />
        </div>
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="h-9 px-3 border border-border rounded-lg text-sm text-foreground bg-surface">
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-sm text-muted-foreground mb-3">Tidak ada tiket maintenance</p>
          <Button size="sm" className="bg-primary hover:bg-primary-80" onClick={() => { setForm(emptyForm); setOpen(true); }}><Plus className="w-4 h-4 mr-1.5" /> Buat Tiket</Button>
        </div>
      ) : (
        <div className="space-y-3">
          {filtered.map((ticket) => {
            ...
            return (
              <Card key={ticket.id} className="border-neutral shadow-sm hover:shadow-md transition-all">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <p className="font-medium text-on-surface truncate">{ticket.title}</p>
                      ...
                      {ticket.description && <p className="text-xs text-tertiary mt-1 truncate">{ticket.description}</p>}
                    ...
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      ...
                      <button type="button" onClick={() => handleDelete(ticket.id)} title="Hapus" className="h-8 w-8 flex items-center justify-center rounded-lg bg-destructive/10 text-destructive hover:bg-red-100 transition-colors cursor-pointer ml-1"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </div>
                </CardContent>
              </Card>
          ...
          <form onSubmit={handleSubmit} className="space-y-3 pt-1">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <select value={form.priority} onChange={(e) => setForm({ ...form, priority: e.target.value })} className="w-full h-9 border border-border rounded-lg px-3 text-sm bg-surface">
            ...
            <div className="space-y-1.5">
              <Label className="text-xs font-medium text-foreground">Aset (opsional)</Label>
              <select value={form.assetId} onChange={(e) => setForm({ ...form, assetId: e.target.value })} className="w-full h-9 border border-border rounded-lg px-3 text-sm bg-surface">
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-neutral">
              ...
              <Button type="submit" className="bg-primary hover:bg-primary-80" disabled={loading}>{loading ? "Menyimpan..." : "Buat Tiket"}</Button>
            </div>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!resolveId} onOpenChange={() => setResolveId(null)}>
        <DialogContent>
          ...
          <div className="space-y-3 pt-1">
            <div className="space-y-1.5"><Label className="text-xs font-medium text-foreground">Catatan Resolusi</Label><Textarea value={resolveNote} onChange={(e) => setResolveNote(e.target.value)} rows={3} placeholder="Deskripsikan tindakan yang diambil..." /></div>
            <div className="flex justify-end gap-2 pt-2 border-t border-neutral">
